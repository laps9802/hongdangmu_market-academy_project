...
public class RegexHelper {
    ...
    public static RegexHelper freeInstance(){
        instance = null;
        return instance;
    }
    // 생성자를 private로 만들면, new에 의한 객체 생성이 불가능해진다.
    private RegexHelper(){}
    // 싱글톤 패턴 종료
     ...
     * @param str : 검사할 문자열
     * @return boolean   : 공백, null이 아닌경우 - true 리턴
     */
    ...
    public  boolean isEmail(String str){
        boolean result = false;
        if(isValue(str)){
            result = Pattern.matches("^[0-9a-zA-A]+(.[_a-z0-9-]+)*@(?:\\w+\\.)+\\w+$", str);
        }
        return result;
    }

    /**
     *  문자열이 핸드폰번호형식으로 구성되었는지 검사
     * @param str : 검사할 문자열
     * @return boolean : 형식에 맞을 경우 - true, 맞지않을 경우 - false
     */
